...
class SimulationModel : public IObserver {
 public:
  ...
  /**
   * @brief Notifies this observer about a message.
   * @param message The message to be notified about.
   */
  void notify(const std::string& message) const;

  /// @brief Queue of packages waiting for delivery.
  std::deque<Package*> scheduledDeliveries;

  std::map<std::string, int> deliveryCounts = {{"astar", 0},
                                              {"bfs", 0},
                                              {"dfs", 0},
                                              {"dijkstra", 0}};
 ...
 protected:
  ...
  const int maxDeliveries = std::numeric_limits<int>::max();
};
