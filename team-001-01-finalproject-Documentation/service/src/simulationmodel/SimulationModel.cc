...
IEntity* SimulationModel::createEntity(const JsonObject& entity) {
  ...
  std::cout << std::endl << name << ": " << position << std::endl;
  ...
  for (auto& [id, entity] : entities) {
    if (name + "_package" == entity->getName()) {
      if (Package* p = dynamic_cast<Package*>(entity)) {
        std::cout << "Found package: " << entity->getName() << std::endl;
        std::cout << "Requires delivery: "
                  << p->requiresDelivery()
                  << std::endl;
...
void SimulationModel::update(double dt) {
  ...
  bool done = true;
  for (auto& [strat, count] : deliveryCounts) {
  ...
  if (done) {
    for (auto& [strat, count] : deliveryCounts) {
      std::cout << strat << ": " << count << std::endl;
    }
    controller.sendEventToView("StopSimulation", JsonObject());
